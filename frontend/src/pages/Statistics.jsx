...
export default function Statistics() {
  ...
  if (loading) return <p className="text-center py-10 text-slate-600">Loading statistics…</p>
  ...
  const inputCls =
    "mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 placeholder:text-slate-400 outline-none transition focus:border-[#0097a7] focus:ring-4 focus:ring-[#0097a7]/20"

  const primaryBtn =
    "inline-flex items-center justify-center rounded-xl bg-[#0097a7] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#007e89] focus:outline-none focus:ring-4 focus:ring-[#0097a7]/25"

  const secondaryBtn =
    "inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-slate-200"

  const tabActive = primaryBtn
  const tabInactive =
    "text-sm font-semibold text-[#0097a7] hover:text-[#007e89] hover:underline"
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 sm:p-6 mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-slate-700">Year from</span>
            <input
              className={inputCls}
              value={yearFrom}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-slate-700">Year to</span>
            <input
              className={inputCls}
              value={yearTo}
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-2">
            ...
            <button type="button" className={secondaryBtn} onClick={resetFilters}>
              Reset
            </button>
          </div>
          ...
          {showLaureatesToggle && (
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                className={mode === "prizes" ? tabActive : tabInactive}
              ...
              <button
                type="button"
                className={mode === "laureates" ? tabActive : tabInactive}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
        <div className="bg-[#0097a7]/10 text-slate-900 px-6 py-4 rounded-2xl shadow-sm text-center border border-[#0097a7]/20">
          <p className="text-2xl font-semibold">🏆 {stats.totalPrizes}</p>
          <p className="text-slate-600 text-sm mt-1">Total Prizes</p>
        </div>
        <div className="bg-[#0097a7]/10 text-slate-900 px-6 py-4 rounded-2xl shadow-sm text-center border border-[#0097a7]/20">
          <p className="text-2xl font-semibold">👩‍🔬 {stats.totalLaureates}</p>
          <p className="text-slate-600 text-sm mt-1">Total Laureates</p>
        </div>
      </div>
      ...
      {showPieChart ? (
        ...
        <div className="flex justify-center mb-10">
          <div className="w-full bg-white border border-slate-200 rounded-2xl shadow-sm p-6 text-center">
            <p className="text-sm text-slate-600 mb-1">{appliedCategoryLabel}</p>
            <p className="text-4xl font-semibold text-[#0097a7]">{selectedValue}</p>
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
        {chartData.map((cat) => (
          <li
            key={cat.name}
            className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex justify-between items-center hover:shadow-md transition"
          >
            <span className="capitalize font-medium text-slate-700">{cat.name}</span>
            <span className="font-semibold text-[#0097a7]">{cat.value}</span>
          </li>
        ))}
      </ul>
    </div>
